...
copy_or_make() {
  ...
  ffmpeg -y -hide_banner -loglevel error \
    -f lavfi -i "mandelbrot=size=${w}x${h}:rate=24:maxiter=90" \
    -t 2.6 \
    -vf "hue=h=${hue}:s=1.35,eq=contrast=1.08:brightness=-0.1,format=yuv420p" \
...
copy_or_make "$OUT/clip-static.mp4" "$WECHAT/2297_1787921505.mp4" 720 1280 25
copy_or_make "$OUT/clip-dolly-in.mp4" "$WECHAT/2295_1787921500.mp4" 960 720 95
...
copy_or_make "$OUT/clip-track.mp4" "$WECHAT/2291_1787921492.mp4" 540 960 145
...
copy_or_make "$OUT/clip-market.mp4" "${track[0]:-}" 1280 720 320
