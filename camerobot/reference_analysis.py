...
Subject geometry comes from the pluggable vision backend (heuristic or YOLO).
Composition / lighting heuristics remain replaceable while preserving
`ReferenceAnalysis` shape.
"""

from __future__ import annotations

from camerobot.models import Intent, ReferenceAnalysis, ReferenceAsset
from camerobot.vision.backend import detection_to_subject_dict, get_vision_backend
...
def analyze_reference(asset: ReferenceAsset, intent: Intent) -> ReferenceAnalysis:
    ...
    detection = get_vision_backend().detect_primary_subject(asset, intent)
    subject = detection_to_subject_dict(detection)
    ...
    return ReferenceAnalysis(
        ...
        confidence=round(
            ...
            + (0.2 if asset.width and asset.height else 0.05)
            + 0.15,
            3,
        ),
    )
